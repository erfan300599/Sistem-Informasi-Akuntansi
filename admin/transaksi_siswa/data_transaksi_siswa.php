<div class="card card-info">
	<div class="card-header">
		<h3 class="card-title">
			<i class="fa fa-table"></i> Data Pembayaran Siswa
		</h3>
	</div>
	<!-- /.card-header -->
	<div class="card-body">
			...
			<table id="example1" class="table table-bordered table-striped">
				...
				<tbody>
					...
					if (isset($_POST['filter'])) {
						...
						$no = 1;
						$sql = $koneksi->query("SELECT tb_siswa.id_siswa, tb_siswa.nisn, tb_siswa.nama_siswa, tb_siswa.telepon_siswa, tb_siswa.jenis_kelamin, tb_siswa.alamat_siswa, tb_siswa.status_siswa, tb_kelas.nama_kelas, tb_jurusan.nama_jurusan FROM tb_siswa
			  	...
			  	INNER JOIN tb_jurusan on tb_jurusan.id_jurusan = tb_siswa.id_jurusan WHERE tb_siswa.id_kelas ='$id_kelas' AND tb_siswa.id_jurusan ='$id_jurusan'");
					...
					?>
							...
							<tr>
								...
								<td>
									<a href="?page=detail-transaksi-siswa&kode=<?php echo $data['id_siswa']; ?>" title="Detail Pembayaran" class="btn btn-primary btn-sm">
										<i class="fa fa-eye"></i>
									</a>
								</td>
							</tr>
						...
						$sql = $koneksi->query("SELECT tb_siswa.id_siswa, tb_siswa.nisn, tb_siswa.nama_siswa, tb_siswa.telepon_siswa, tb_siswa.jenis_kelamin, tb_siswa.alamat_siswa, tb_siswa.status_siswa, tb_kelas.nama_kelas, tb_jurusan.nama_jurusan FROM tb_siswa
			  	INNER JOIN tb_kelas ON tb_kelas.id_kelas = tb_siswa.id_kelas
			  	INNER JOIN tb_jurusan on tb_jurusan.id_jurusan = tb_siswa.id_jurusan WHERE tb_siswa.id_siswa");
						while ($data = $sql->fetch_assoc()) {

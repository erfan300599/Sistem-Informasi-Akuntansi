...
<html>
<head>
<title>Cetak - Laporan SPP Siswa</title>
   <style type="text/css">
...
//          WHERE tb_siswa.id_kelas=tb_kelas.id_kelas and id_siswa='$_GET[id_siswa]' AND status_siswa='Aktif' ORDER BY nama_siswa ASC");
...
<div align="center">
<h3>PEMBAYARAN SUMBANGAN PEMBINAAN PENDIDIKAN (SPP)</h3>
</div>
...
<table border="1" width="100%" class="tabel">
   ...
   <tbody>
         ...
         $sqlJenisBayar =  $koneksi->query("SELECT * from tb_jenis_bayar_siswa
            WHERE id_tahun_ajaran='$id_tahun_ajaran' and tipe_jenis_bayar_siswa='bulanan'");
               while($djb=$sqlJenisBayar->fetch_assoc()){
             $sql = $koneksi->query("SELECT tb_jenis_bayar_siswa.nama_bayar_siswa, tb_siswa.nama_siswa, tb_bulan.nama_bulan, tb_bulan.urutan, tb_tagihan_bulanan_siswa.jml_bayar, SUM(tb_tagihan_bulanan_siswa.jml_bayar) as totaljmlbayar2, SUM(tb_tagihan_bulanan_siswa.terbayar) as totalterbayar FROM tb_jenis_bayar_siswa 
                 left JOIN tb_tahun_ajaran ON tb_jenis_bayar_siswa.id_tahun_ajaran=tb_tahun_ajaran.id_tahun_ajaran 
                 left JOIN tb_tagihan_bulanan_siswa on tb_tagihan_bulanan_siswa.id_jenis_bayar_siswa=tb_jenis_bayar_siswa.id_jenis_bayar_siswa
                 left JOIN tb_siswa ON tb_tagihan_bulanan_siswa.id_siswa=tb_siswa.id_siswa
                 left JOIN tb_bulan ON tb_tagihan_bulanan_siswa.id_bulan = tb_bulan.id_bulan 

                     where 
                     tb_tagihan_bulanan_siswa.id_jenis_bayar_siswa='$djb[id_jenis_bayar_siswa]' and
                     tb_tagihan_bulanan_siswa.id_siswa='$id_siswa' 
                   GROUP BY tb_tagihan_bulanan_siswa.id_tagihan_bulanan_siswa");

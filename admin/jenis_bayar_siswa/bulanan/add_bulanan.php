...
?>
	...
	<div class="card card-warning">
		<div class="card-header">
			<h3 class="card-title">
				<i class="fa fa-edit"></i> Pilih Kelas dan Input Pembayaran
			</h3>
		</div>
						...
						<div class="form-group row">
							...
							<div class="col-sm-6">
								<div class="input-group">
									...
									<input type="text" class="form-control uang" name="tarif" id="txtFirst" onkeyup="copytextbox();" autocomplete="off" required>
								</div>
							</div>
						</div>
						<script>
							function copytextbox() {
								var value = document.getElementById('txtFirst').value.replace(/\./g, ''); // Hapus titik yang sudah ada
								var formattedValue = addCommas(value); // Tambahkan titik setiap tiga digit
								document.getElementById('txtFirst').value = formattedValue;
								// Tambahkan kode berikut jika Anda ingin mengcopy nilai ke field lainnya
								document.getElementById('n1').value = formattedValue;
								document.getElementById('n2').value = formattedValue;
								document.getElementById('n3').value = formattedValue;
								document.getElementById('n4').value = formattedValue;
								document.getElementById('n5').value = formattedValue;
								document.getElementById('n6').value = formattedValue;
								document.getElementById('n7').value = formattedValue;
								document.getElementById('n8').value = formattedValue;
								document.getElementById('n9').value = formattedValue;
								document.getElementById('n10').value = formattedValue;
								document.getElementById('n11').value = formattedValue;
								document.getElementById('n12').value = formattedValue;
							}
	...
	<div class="card card-warning">
		<div class="card-header">
			<h3 class="card-title">
				<i class="fa fa-table"></i> Data Tagihan Siswa
			</h3>
		</div>
		<!-- /.card-header -->
		<div class="card-body">
                      ...
                      GROUP BY tb_siswa.id_siswa	
						...
						?>
								...
								<tr>
									...
									<td>
										<a href="?page=hapus-bulanan&id_siswa=<?php echo $data['id_siswa']; ?>&id_jenis_bayar_siswa=<?php echo $id_jenis_bayar_siswa ?>" onclick="return confirm('Apakah anda yakin hapus data ini ?')" title="Hapus" class="btn btn-danger btn-sm">
					...
					<div class="form-group row">
						...
						<div class="col-sm-6">
							<div class="input-group">
								...
								<?php
								$tarif = isset($_POST['tarif']) ? floatval($_POST['tarif']) : '';
								$formattedTarif = ($tarif !== '') ? number_format($tarif, 0, ',', '.') : '';
								?>
								<input type="text" class="form-control" name="tarif" id="tarif" value="<?php echo $formattedTarif; ?>" autocomplete="off" required>
							</div>
						</div>
					</div>

					<script>
						// Function to add dot after every three or four digits
						function addDot(input) {
							// Remove existing dots and non-numeric characters
							var value = input.value.replace(/[^\d]/g, '');

							// Determine the grouping size
							var groupingSize = value.length === 5 ? 2 : (value.length >= 6 && value.length <= 8 ? value.length - 3 : 0);

							// Add dots after every grouping size digits
							var formattedValue = '';
							for (var i = 0; i < value.length; i++) {
								if (i > 0 && (i % groupingSize === 0)) {
									formattedValue += '.';
								}
						...
						tarifInput.addEventListener('focus', function() {
							...
							this.value = value !== '' ? parseInt(value).toString() : '';
						});
                      	...
                      	");
							...
							?>
									...
									<tr>
										...
										<td>
											<a href="?page=hapus-bebas&id_tagihan_bebas_siswa=<?php echo $data['id_tagihan_bebas_siswa']; ?>&id_jenis_bayar_siswa=<?php echo $id_jenis_bayar_siswa ?>" onclick="return confirm('Apakah anda yakin hapus data ini ?')" title="Hapus" class="btn btn-danger btn-sm">
												<i class="fa fa-trash"></i>
												<a />
										</td>
									</tr>
